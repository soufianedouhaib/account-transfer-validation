...
module.exports = {
  ...
  /* Every account in this build is a demo account, so each door offers its own
     set for one click sign in: three employees on one, two managers and the
     administrator on the other. A button on each page crosses to the other, and
     either door still accepts any account and sends the person to the home
     their role has, so a mistyped door is never a dead end. Swap APP_USERS in
     and the lists vanish. */
